...
#### 1. Lazy Model Loading in Dataloader Workers
**Impact:** Saves ~19GB memory per worker for large models

**Current issue:**
- Workers receive full pickled copy of `self.sd` (StableDiffusion model)
- For Qwen-Image: 19GB × num_workers memory waste

**Proposed solution:**
```python
class AiToolkitDataset:
    def __init__(self, sd=None):
        # Store model config instead of full model
        self.model_config = sd.model_config if sd else None
        self.model_path = sd.model_path if sd else None
        self.sd = None  # Don't store model reference

    def _ensure_model_loaded(self):
        """Lazy load model in worker on first access"""
        if self.sd is None and is_worker_process():
            self.sd = load_model_from_config(self.model_config, self.model_path)
```

**Files to modify:**
- `toolkit/data_loader.py`: AiToolkitDataset.__init__
- `toolkit/dataloader_mixins.py`: All mixins that access self.sd
- Add `is_worker_process()` utility function

**Complexity:** Medium
**Estimated savings:** 19GB per worker
